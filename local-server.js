...
function loadBookingStore() {
  try {
    ...
    if (!Array.isArray(store.blogPosts)) store.blogPosts = [...BLOG_SEED_POSTS];
    if (!store.blogAutomation || typeof store.blogAutomation !== 'object') store.blogAutomation = { nextArcanaIndex: 1, lastPostedWeekKey: null };
    ...
    return {
      ...
      referralCodes: {},
      bonusReadingCredits: {},
      blogPosts: [...BLOG_SEED_POSTS],
      blogAutomation: { nextArcanaIndex: 1, lastPostedWeekKey: null },
      astralChartsByEmail: {},
...
// Blog editorial -- ver el espejo completo y comentado en
// netlify/functions/booking.mts (BLOG_SEED_POSTS).
...
// Blog automatico semanal -- ver el espejo completo y comentado en
// netlify/functions/booking.mts (generateWeeklyBlogPost). En local no
// hay cron real (ver la accion generate-weekly-blog-post mas abajo,
// gateada por requireAdmin en vez de CRON_SECRET).
...
async function generateWeeklyBlogPost(store) {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) {
    const err = new Error('Falta GROQ_API_KEY para generar la nota semanal del blog.');
    err.isConfig = true;
    throw err;
  }
  const cardsData = loadTarotCardsData();
...
CUERPO_EN: <paragraph 1><<<PARA>>><paragraph 2><<<PARA>>><paragraph 3><<<PARA>>><paragraph 4>`;
  ...
  const aiResult = await PROVIDERS.groq.call(prompt, 2200, 'openai/gpt-oss-120b', apiKey);
  const text = aiResult.text;
  try {
    logAiUsage(store, { kind: 'blog-weekly', provider: 'groq', model: 'openai/gpt-oss-120b', inputTokens: aiResult.inputTokens, outputTokens: aiResult.outputTokens });
  } catch (e) {
  ...
  const bodyOf = (label) => pick(label, '').split('<<<PARA>>>').map((s) => s.trim()).filter(Boolean);
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (action === 'blog-posts') {
        return sendJson(res, 200, { posts: store.blogPosts || [] });
      }
    ...
    if (action === 'login') {
      ...
      if (!user || !verifyAccountPassword(pw, user.passwordSalt, user.passwordHash)) {
        ...
        if (!user && store.pendingSignups[email]) {
          return sendJson(res, 401, { error: 'Todavía no confirmaste tu email — revisa tu bandeja de entrada (o spam) y haz clic en el link que te enviamos.' });
        }
    ...
    if (action === 'generate-weekly-blog-post') {
      // En local no hay Netlify Scheduled Functions -- este es solo el
      // botón manual (requireAdmin) para poder probar el flujo entero,
      // sin el chequeo de "viernes 10am" (ver weekly-blog-cron.mts /
      // generate-weekly-blog-post en booking.mts para el cron real).
      if (!requireAdmin(store, payload, action)) return sendJson(res, 401, { error: 'Contraseña de administración incorrecta o faltante.' });
      try {
        const { post, idx } = await generateWeeklyBlogPost(store);
        store.blogPosts.push(post);
        store.blogAutomation.nextArcanaIndex = idx + 1;
        store.blogAutomation.lastPostedWeekKey = santiagoDateKey();
        saveBookingStore(store);
        return sendJson(res, 200, { ok: true, slug: post.slug });
      } catch (e) {
        if (e.isConfig) return sendJson(res, 200, { skipped: true, reason: 'groq-not-configured' });
        return sendJson(res, 500, { error: e.message || 'No se pudo generar la nota semanal.' });
      }
    }
