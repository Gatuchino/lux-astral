...
function App() {
  ...
  // First-run: launch onboarding automatically if user hasn't finished it.
  const onboarded = (() => {
    try { return !!JSON.parse(localStorage.getItem('vela_onboarding') || 'null'); } catch { return false; }
  })();
  const [route, setRouteState] = React.useState(() => {
    ...
    return onboarded ? { page: 'home' } : { page: 'onboarding' };
  });
  ...
  React.useEffect(() => {
    if (!profile.email) { setPlanInfo(null); return; }
    let cancelled = false;
    window.arcanaSubscriberStatus(profile.email)
  ...
  React.useEffect(() => {
    if (!profile.email) { setIsPowerUser(false); return; }
    let cancelled = false;
    window.arcanaPowerUserStatus(profile.email)
  ...
  React.useEffect(() => {
    window.arcanaLogEvent(profile.email, route.page).catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [route.page, profile.email]);
  ...
  React.useEffect(() => {
    if (!profile.email || !isSubscriber) { setReadings([]); return undefined; }
    let cancelled = false;
    window.arcanaListReadings(profile.email)
      .then((res) => {
        if (cancelled) return;
        if (!res.migrated) {
          let local = [];
          ...
          if (local.length) {
            window.arcanaImportReadings(profile.email, local)
  ...
  const saveReading = (reading) => {
    ...
    if (profile.email) {
      window.arcanaSaveReading(profile.email, reading).catch((e) => console.error('[Arcana] no se pudo guardar la lectura en el servidor', e));
    }
  };
  ...
  const deleteReading = (id) => {
    ...
    if (profile.email) {
      window.arcanaDeleteReading(profile.email, id).catch((e) => console.error('[Arcana] no se pudo borrar la lectura en el servidor', e));
    }
  };
  ...
  const updateReading = (id, patch) => {
    ...
    if (profile.email) {
      window.arcanaUpdateReading(profile.email, id, patch).catch((e) => console.error('[Arcana] no se pudo actualizar la lectura en el servidor', e));
    }
  };
  ...
  switch (route.page) {
    ...
    case 'marketplace': page = <MarketplacePage lang={lang} setRoute={setRoute} />; break;
